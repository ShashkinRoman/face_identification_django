...
# ws.connect("ws://192.168.0.139:9000", max_size=1000000000000)
#
#
#
...
known_face_encodings = [
    roman_face_encoding,
    roman_in_glasses_face_encoding,
...
def path_image_func():
    query = Access_status.objects.all()
    for i in query:
        path_image_dict[i.name] = i.path_image
...
# def screen_save(name_, frame_):
#     # date_ = datetime.now().strftime("%d/%m/%y %H:%M:%S")
#     date_ = datetime.now()
#     a = cv2.imwrite('screenshots/test_{}_{}.png'.format(name_, date_), frame_)
#     path = 'screenshots/test_{}_{}.png'.format(name_, date_)
#     return path, a
...
def show_frame(face_locations, name_, frame_ ):
    ...
    date_ = datetime.now()
    date_for_path = date_.strftime("%d/%m/%y_%H:%M:%S")
